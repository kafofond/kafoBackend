...
public class UtilisateurController {
     ...
     * @param entrepriseId (optionnel) ID de l'entreprise à filtrer (SUPER_ADMIN uniquement)
     */
    @GetMapping
    public ResponseEntity<?> listerUtilisateurs(
        ...
        try {
            log.info("Liste des utilisateurs demandée par {} (entrepriseId: {})", authentication.getName(), entrepriseId);
            ...
            if (entrepriseId != null) {
                ...
                log.info("Vérification du rôle pour filtrage par entreprise. Rôle de l'utilisateur: {}, Rôle requis: {}", 
                         utilisateur.getRole(), Role.SUPER_ADMIN);
                if (utilisateur.getRole() != Role.SUPER_ADMIN) {
                    log.warn("Accès refusé: l'utilisateur {} avec le rôle {} n'est pas autorisé à filtrer par entreprise", 
     ...
     */
    @PostMapping
    public ResponseEntity<?> creerUtilisateur(@RequestBody UtilisateurDTO utilisateurDTO, Authentication authentication) {
        try {
            ...
            // Vérifier si l'utilisateur a les autorisations nécessaires directement avec Spring Security
            ...
            // Créer l'utilisateur directement sans vérification de rôle dans le service
            Utilisateur nouvelUtilisateur = utilisateurMapper.toEntity(utilisateurDTO);
            
            // Si l'administrateur est un SUPER_ADMIN et qu'un ID d'entreprise est fourni dans le DTO, l'associer
            if (admin.getRole() == Role.SUPER_ADMIN && utilisateurDTO.getEntrepriseId() != null) {
                // Ne pas créer l'entreprise manuellement, laisser le service gérer cela
                // L'ID de l'entreprise est déjà dans l'objet utilisateur via le mapper
                log.info("Entreprise associée à l'utilisateur: ID={}", utilisateurDTO.getEntrepriseId());
            }
            
            // Si l'administrateur est un ADMIN, associer automatiquement son entreprise
            if (admin.getRole() == Role.ADMIN) {
                nouvelUtilisateur.setEntreprise(admin.getEntreprise());
                log.info("Entreprise associée à l'utilisateur: ID={}", admin.getEntreprise().getId());
            }
            
            // Appeler le service avec la vérification de rôle désactivée
            Utilisateur utilisateurCree = utilisateurService.creerUtilisateur(nouvelUtilisateur, admin);
            UtilisateurDTO utilisateurCreeDTO = utilisateurMapper.toDTO(utilisateurCree);
            
            // Récupérer le nom de l'entreprise de manière transactionnelle si nécessaire
            if (utilisateurCreeDTO.getEntrepriseId() != null && utilisateurCreeDTO.getEntrepriseNom() == null) {
                String nomEntreprise = utilisateurService.getNomEntrepriseParId(utilisateurCreeDTO.getEntrepriseId());
                utilisateurCreeDTO.setEntrepriseNom(nomEntreprise);
            }
    ...
    public ResponseEntity<?> modifierUtilisateur(@PathVariable Long id, @RequestBody UtilisateurDTO utilisateurDTO, 
        ...
        try {
            ...
            UtilisateurDTO utilisateurMisAJourDTO = utilisateurService.modifierUtilisateurDTO(id, utilisateurModifie, admin);
        ...
        try {
            ...
            if (utilisateur.getRole() != Role.SUPER_ADMIN) {
                ...
                if (utilisateur.getRole() == Role.ADMIN) {
                    List<UtilisateurDTO> utilisateursDTO = utilisateurService.listerParEntrepriseDTO(utilisateur.getEntreprise());
     ...
     * @param entrepriseId (optionnel) ID de l'entreprise à filtrer (SUPER_ADMIN uniquement)
     * @param tous (optionnel) Si true, liste tous les actifs de l'application (SUPER_ADMIN uniquement)
     */
    @GetMapping("/actifs")
        ...
        try {
            ...
            if (entrepriseId != null) {
                ...
                if (utilisateur.getRole() != Role.SUPER_ADMIN) {
                    ...
                    if (utilisateur.getRole() == Role.ADMIN && utilisateur.getEntreprise().getId().equals(entrepriseId)) {
     ...
     * @param entrepriseId (optionnel) ID de l'entreprise à filtrer (SUPER_ADMIN uniquement)
     * @param tous (optionnel) Si true, liste tous les inactifs de l'application (SUPER_ADMIN uniquement)
     */
    @GetMapping("/inactifs")
        ...
        try {
            ...
            if (entrepriseId != null) {
                ...
                if (utilisateur.getRole() != Role.SUPER_ADMIN) {
                    ...
                    if (utilisateur.getRole() == Role.ADMIN && utilisateur.getEntreprise().getId().equals(entrepriseId)) {
                        utilisateursDTO = utilisateurService.listerParRoleEtEntrepriseDTO(roleEnum, utilisateur.getEntreprise());
                    } else {
                        Map<String, String> error = new HashMap<>();
                        error.put("message", "Seul le SUPER_ADMIN peut filtrer par entreprise");
                        return ResponseEntity.status(403).body(error);
    ...
    public ResponseEntity<?> modifierMonProfil(@RequestBody UtilisateurDTO utilisateurDTO, 
        ...
        try {
            ...
            Utilisateur utilisateurModifie = utilisateurMapper.toEntity(utilisateurDTO);
            UtilisateurDTO utilisateurMisAJourDTO = utilisateurService.modifierUtilisateurDTO(utilisateur.getId(), utilisateurModifie, utilisateur);
    ...
    public ResponseEntity<?> modifierMotDePasseUtilisateur(
        ...
        try {
            ...
            UtilisateurDTO utilisateurModifie = utilisateurService.modifierMotDePasseUtilisateur(id, nouveauMotDePasse, admin);
